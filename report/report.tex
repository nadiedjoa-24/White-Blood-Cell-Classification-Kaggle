...
\textbf{Part~II} traces nine successive deep-learning iterations (\texttt{deep\_1}--\texttt{deep\_9}), each motivated by specific failures of the previous one. Starting from a vanilla EfficientNet-B3 baseline (single 85/15 validation split, macro-F1~=~0.552), we explored CBAM attention, late fusion with handcrafted morphometry, Wiener/Laplacian preprocessing, paper-based segmentation pipelines, before pivoting to ConvNeXt-Base with Focal Loss, CutMix/Mixup, and 2-phase fine-tuning. We document a critical data leakage pitfall (\texttt{deep\_5}: apparent F1=0.82, corrected to true F1=0.70 in \texttt{deep\_6}), and present post-hoc experiments with ResNet50 (\texttt{deep\_7}), ResNeXt101 with multi-task learning (\texttt{deep\_8}), and a controlled ablation of handcrafted features (\texttt{deep\_9}). The deployed model is \texttt{deep\_9 no-morph} (no handcrafted features): \texttt{deep\_6}'s architecture plus a lightweight SE-style attention gate and HSV-V CLAHE preprocessing, achieving macro-F1 = 0.7052 / accuracy = 0.937 -- the best validated result across all 9 iterations.

Beyond raw scores, this work establishes that (1)~learned representations dominate handcrafted features ($+0.19$ F1, \texttt{deep\_6} vs.\ classical ML), (2)~the backbone family (ConvNeXt) mattered more than added modules: a similarly sized ResNeXt101 with CBAM and multi-task heads (\texttt{deep\_8}, 0.665) did not reach the plain ConvNeXt + MLP (0.699), and attention/fusion gave at most marginal gains, and (3)~the most impactful engineering decisions are at the \emph{data} level (offline cropping, leakage-safe CV, rare-class-aware augmentation).
...
The challenge dataset consists of 28,901 training images and 9,634 test images of individual white blood cells captured from peripheral blood smear microscopy. Images are RGB PNGs of roughly $370\times370$ pixels (small size variations, e.g.\ $368$--$370 \times 368$). Each image shows a single WBC against a background of red blood cells and platelets, labeled into 13 cell types ranging from mature granulocytes (segmented neutrophils, eosinophils, basophils) to immature precursors (blasts, promyelocytes, myelocytes, metamyelocytes, band neutrophils), agranulocytes (lymphocytes, monocytes), reactive forms (variant lymphocytes), and rare entities (plasma cells, platelet clumps). The metadata files contain only \texttt{ID} and \texttt{label}, so the classifier must rely exclusively on the visual content.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{class_examples.pdf}
...
Staining variability across slides causes significant color shifts that can confuse both segmentation algorithms and classifiers. We apply Reinhard color normalization~\cite{reinhard2001} in LAB color space: for each image, channel-wise mean and standard deviation are matched to a reference computed from a stratified sample of 40 images per class. This standardizes the color distribution while preserving morphological structure.
...
Features are standardized (zero mean, unit variance) and class imbalance is handled via SMOTE~\cite{chawla2002smote} (k=3 neighbors) at each cross-validation fold. We evaluate 6 classifiers with 5-fold stratified cross-validation. \textbf{Computational note}: the literal SMOTE call equalizes every class up to the majority class count (SNE, 13{,}015), which would inflate each training fold to $\sim$135k rows; at that scale, benchmarking showed the SVM-based models (SVM-RBF, Voting, Stacking) would require multiple hours to days to fit on this hardware (4 CPU cores). We therefore cap SMOTE oversampling at 2{,}000 samples/class (\texttt{SMOTE\_CAP} in \texttt{src/classical\_pipeline.py}) instead of equalizing to the majority class --- documented here as a deliberate adaptation for feasibility, not a change to any other part of the pipeline:

\begin{itemize}
    \item \textbf{LightGBM}~\cite{chen2017lightgbm} (tuned via Optuna~\cite{akiba2019optuna}: 1755 estimators, lr=0.034, 54 leaves, depth 6)
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{crop_examples.pdf}
...
\begin{enumerate}
    \item \textbf{Offline oversampling}: The 6 rarest classes (below median count 415) are augmented to 415 images each via random geometric transforms.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{augmentation_gallery.pdf}
...
\texttt{deep\_6} & ConvNeXt-Base & Leakage fix, visual QC & 0.700 & DL & True performance after leakage fix \\
...
\textbf{Results}: True macro-F1 = 0.6994, accuracy = 0.936. The 12.3-point absolute drop from \texttt{deep\_5} (0.822 $\to$ 0.699, $\approx$15\% relative) demonstrates the severity of the leakage. Training reached epoch 21 (early stopping, patience=5), best at epoch 16. A hybrid approach (backbone features $\to$ Random Forest) was tested but underperformed (F1 = 0.68).

This establishes the leakage-safe architecture and evaluation protocol that all later iterations (\texttt{deep\_7}--\texttt{deep\_9}) build on. It is not, however, the best-scoring iteration: \texttt{deep\_9 no-morph}, which adds one lightweight attention gate and HSV-V CLAHE on top of this architecture, scores slightly higher (0.7052, \S\ref{sec:iter_deep9}) and is the model actually deployed.
...
\textbf{Results}: 5-fold CV mean F1 = 0.646 ($\pm$0.018), holdout F1 = 0.651. Ablation showed morpho features added +0.017 F1 over image-only (0.633 $\to$ 0.651). The hybrid Features+RF scored 0.593 (worse than end-to-end). Per-class: BNE=0.49, PMY=0.32, PLY=0.00 remained critical.

\textbf{Key insight}: ResNet50 (24.8M params) is significantly weaker than ConvNeXt-Base (88M params) for this task. The morphological features provide a small but inconsistent benefit with this backbone.
...
\textbf{Results}: 5-fold CV mean F1 = 0.680 ($\pm$0.022), holdout F1 = 0.665. A zero-out ablation---morphological inputs set to zero \emph{at inference only}, on the model trained with them (no retraining)---dropped F1 from 0.665 to 0.492 ($-0.173$). This measures how much the trained network \emph{relies} on its morphology branch, not the gain that adding the features would bring over a model trained without them. However, the overall F1 (0.665) still did not exceed \texttt{deep\_6} (0.700). Per-class: BA=0.96, EO=0.96, but BNE=0.42, PMY=0.38, PLY=0.00.

\textbf{Key insight}: With a backbone of essentially the same size as ConvNeXt-Base ($\sim$87M vs.\ $\sim$88M params), plus CBAM attention and multi-task supervision, the result still does not beat the simpler ConvNeXt-Base + MLP. Parameter count alone does not explain the gap; the backbone family/architecture mattered more than the added modules.

\subsection{\texorpdfstring{\texttt{deep\_9}}{deep\_9}: ConvNeXt-Base + SE Attention (with-morph vs.\ no-morph)}
\label{sec:iter_deep9}

\textbf{Motivation}: Definitive controlled experiment---does adding handcrafted morphological features help when using ConvNeXt-Base?
...
\begin{itemize}
    ...
    \item \texttt{deep\_9 no-morph}: val macro-F1 = \textbf{0.7052}, accuracy = 0.9367 --- the best validated macro-F1 across all 9 iterations, and the model deployed (checkpoint at \texttt{models/best\_model.pt}, \S\ref{sec:deployed})
\end{itemize}

The morphological features make \textit{essentially no difference} (0.7036 vs.\ 0.7052, a 0.0016 macro-F1 difference, too small to be meaningful on a single validation split). Both are comparable to \texttt{deep\_6} (0.6994), indicating that ConvNeXt-Base already captures the morphological information that these handcrafted features provide. A Features+RF hybrid on the gated \texttt{deep\_9 no-morph} features reached 0.6801, again below end-to-end training. The dominant confusions remain the same: for \texttt{deep\_9 no-morph}, BNE$\rightarrow$SNE accounts for 41.0\% of BNE and VLY$\rightarrow$LY for 24.3\% of VLY.

\textbf{Key insight}: This is the definitive answer to ``do handcrafted features help?''---\textit{not with this backbone.} ConvNeXt-Base learns representations that subsume explicit morphometry. With the other backbones (EfficientNet-B3, ResNet50, ResNeXt101) the fused models did make use of the morphology branch (\texttt{deep\_7} $+0.017$; \texttt{deep\_8} relies on it, see above), but none of them reached ConvNeXt's level. Since \texttt{deep\_9 no-morph} scores marginally higher and is simpler (no unused morphology branch), it is the deployed configuration.
...
\begin{table}[h]
\centering
\caption{Comparison of approaches (validation macro-F1).}
...
\vspace{2pt}
{\footnotesize $^\dagger$Reduced protocol (single 85/15 split, 15-epoch cap) -- see \S\ref{sec:iter_deep1}, not directly comparable to the other two rows' full training budgets.}
\end{table}
...
These confusions reflect genuine biological continua, not model failures.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{qualitative_errors.pdf}
...
Across these examples, errors are largely explained by borderline morphology (e.g., partially segmented nucleus, intermediate chromatin condensation, weak granulation) rather than obvious model artifacts. This supports the interpretation that a substantial fraction of residual error reflects class overlap in the biological continuum.

\subsection{Why Deep Learning Outperforms Classical ML}

The performance gap (0.196 F1, 0.705 vs.\ 0.509) arises from three factors:
...
\begin{enumerate}
    \item \textbf{The backbone family matters more than added modules}: The single largest performance jump came from switching EfficientNet-B3 (12M) to ConvNeXt-Base (88M). Size alone is not the explanation: ResNeXt101 ($\sim$87M, \texttt{deep\_8}) with CBAM, fusion and multi-task heads reached only 0.665. No amount of attention, fusion, or multi-task engineering compensated for a less suitable backbone.
    \item \textbf{Handcrafted features matter for weaker pipelines, not for ConvNeXt}: Late fusion gave small gains with EfficientNet-B3 (\texttt{deep\_3} vs.\ \texttt{deep\_2}, $+0.008$) and ResNet50 (\texttt{deep\_7}, $+0.017$), and the ResNeXt101 model strongly relied on its morphology branch (zero-out ablation $-0.173$ at inference), but it provided no benefit for ConvNeXt-Base (\texttt{deep\_9} with-morph vs.\ no-morph: 0.0016 difference).
    \item \textbf{Data leakage is insidious}: The \texttt{deep\_5} $\to$ \texttt{deep\_6} episode (0.82 $\to$ 0.70) demonstrates how augmentation+CV can create subtle leakage. Always split by original image identity.
    \item \textbf{Simplicity wins, mostly}: The deployed model (\texttt{deep\_9 no-morph}) is \texttt{deep\_6}'s backbone + MLP plus one lightweight SE gate and CLAHE preprocessing -- the only additions that helped at all, and only marginally ($+0.006$ F1, 0.7052 vs.\ 0.6994). Heavier designs (CBAM, spatial attention, multi-task, late fusion) added engineering complexity without consistent gains.
...
\begin{figure}[h]
    ...
    \caption{Evolution of validation macro-F1 across iterations. The \texttt{deep\_5}$\to$\texttt{deep\_6} drop reflects leakage correction, not regression.}
...
\begin{enumerate}
    ...
    \item \textbf{Lightweight SE-style attention gate} on pooled features (263K params) -- together with CLAHE, the only addition that gave a (marginal) gain.
...
This achieves macro-F1 = 0.705 on validation, outperforming classical ML (0.509) and the initial DL baseline (0.552). The iterative journey demonstrated that \textit{the choice of backbone family and training robustness matter more than architectural complexity} -- attention and fusion mostly did not help, with one narrow exception.
